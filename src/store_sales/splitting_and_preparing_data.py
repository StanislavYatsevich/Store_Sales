...
@click.command()
@click.option('--raw_data_folder_path', default=RAW_DATA_FOLDER_PATH, type=click.Path(exists=True), help='Path to the folder with raw data files')
@click.option('--prepared_data_folder_path', default=PREPARED_DATA_FOLDER_PATH, type=click.Path(), help='Path to the folder where prepared files are saved to')
...
def split_and_prepare_data(raw_data_folder_path, prepared_data_folder_path):
    ...
    holidays_events_data = pd.read_csv(Path(raw_data_folder_path)/'holidays_events.csv')
